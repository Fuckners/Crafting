...
class Item {
    constructor(nome, classe, crafting, url='') {
        this.itemImage = 'imagens/' + url, // imagens/la_branca.png
        // this.itemQuant = qtd, // 1
        this.itemName = nome, // pedra lisa
        this.itemClass = classe, // pedra
        // this.itemId = `#${nome.toLowerCase()}`,
...
const blue_bed = new Item('Cama Azul', 'bed', 'none none none whool whool whool plank plank plank', 'Cama_azul.webp');
console.table(blue_bed);

const chest = new Item('Baú', 'chest', 'plank plank plank plank none plank plank plank plank', 'Bau.webp');
console.table(chest);

const craft_table = new Item('Bancada de Trabalho', 'workbanch', 'none none none none plank plank none plank plank', 'CraftingTable.webp');
console.table(craft_table);

// Todos os items criado devem ser adicionados aqui
const items = [
    plank,
    white_whool, blue_whool,
    blue_bed,
    chest,
    craft_table
]
...
Array.from(document.getElementsByClassName('slots'))
.forEach(element => {
...
document.getElementById('trash').addEventListener('dragenter', (event) => event.target.style.opacity = 0.8);

document.getElementById('trash').addEventListener('dragleave', (event) => event.target.style.opacity = 1);
...
(function firstInfyItems(arr) {
    ...
    for (let item of arr) {
        infyBar.innerHTML += 
        `<div class="infy" id="${item.itemName}" ondragover="onDragOver(event)"></div>`;
    }
    infyItems();
})(items);
...
function infyItems() {
    for (let item of items) {
        ...
        let slot = document.getElementById(item.itemName);
        slot.innerHTML = 
        `<div class="item ${item.itemClass}" id="${item.itemName}#${count}" data-name="${item.itemName}" draggable="true" ondragstart="onDragStart(event)">
            <abbr class="name" title="${item.itemName}">
                <span class="qtd">1</span>
                <img src="${item.itemImage}" draggable="false">
            </abbr>
        </div>
        `;
    };
...
function verificarTarget(targetO) {
    ...
    if (targetO.tagName == 'SPAN' || targetO.tagName == 'IMG') {
        ...
        eventTarget = targetO. // SPAN/IMG
        parentElement. // ABBR
        parentElement. // DIV.item
        parentElement // DIV.slot
    ...
    if (eventTarget.id == 'trash') {
        ...
        try {
            ...
            if (draggableElement.parentElement.id == eventTarget.id) {
                ...
                if (draggableElement.dataset.name == eventTarget.firstElementChild.dataset.name) {
...
function craftTable() {
    ...
    for (let item of items) {
        ...
        if (item.crafting == crafts.join(' ')) {
            console.log(`O item ${item.itemName} vai ser craftado!`);
            finishSlot.innerHTML = 
            `<div class="item ${item.itemClass}" id="${item.itemName}#${count}" draggable="true" ondragstart="onDragStart(event)">
                <abbr class="name" title="${item.itemName}">
                    <span class="qtd">1</span>
                    <img src="${item.itemImage}" draggable="false">
                </abbr>
